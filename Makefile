...
pytest: build-image-test
	...
	env > $(TEMP_FILE)
	docker run --interactive --env-file $(TEMP_FILE) --user $$(id -u) --workdir /app  -v $$(pwd):/app $(DOCKER_IMAGE_URL)-test:$(BUILD_VERSION) pytest -vvvv --strict-markers --basetemp=./.pytest_tmp/ --cov-report term-missing --cov=. --cov-fail-under=69.81 --cov-report xml:coverage.xml --cov-report=html
