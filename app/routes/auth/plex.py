...
from . import ns as auth_ns


@auth_ns.route("/plex/check")
class PlexAuthCheck(Resource):
    def get(self):
        """
        ...
        if (
            ...
            user_2fa is True
            and plex_auth_user_session.get("2fa-enabled", False) is False
        ):
            return "access denied", 403
...
@auth_ns.route("/plex/session")
class PlexAuthSession(Resource):
    def get(self):
        ...
        url_parse_redirect = (
            urlparse(redirect_arg) if redirect_arg is not None else None
        )
...
@auth_ns.route("/plex/logout")
class PlexAuthLogout(Resource):
    def get(self):
